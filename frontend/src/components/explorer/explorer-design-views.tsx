...
import { ArrowLeft, ArrowRight, Bell, Check, ChevronDown, ChevronLeft, CircleDollarSign, Columns3, Copy, Eye, Globe2, ListFilter, MoreHorizontal, Play, Receipt, RotateCcw, Send, Share2, X } from "lucide-react";
import type {
...
import { addressRoute, explorerRoutes, isContractAddress, stellarExpertRoute, storedProjectHome, truncateEntity } from "@/lib/explorer-routes";
...
function amountLabel(tx: ExplorerTxDetail) {
  if (tx.status.toLowerCase() === "failed") return "No applied transfer";
  ...
  return first ? `${first.amount} ${displayAsset(first.asset) ?? first.asset}` : "No asset transfer";
}

type ClassicOperation = { [key: string]: JsonValue };

function operationRecords(tx: ExplorerTxDetail): ClassicOperation[] {
  return Array.isArray(tx.operation_details)
    ? tx.operation_details.filter((value) => Boolean(value) && typeof value === "object" && !Array.isArray(value)) as ClassicOperation[]
    : [];
}

function textField(operation: ClassicOperation, key: string) {
  const value = operation[key];
  return typeof value === "string" ? value : null;
}

function assetLabel(type: string | null, code: string | null, issuer: string | null) {
...
function operationAsset(operation: ClassicOperation, prefix: "source_" | "") {
  return assetLabel(
    ...
    textField(operation, `${prefix}asset_code`),
    textField(operation, `${prefix}asset_issuer`),
  );
}

function attemptedPath(operation: ClassicOperation) {
  const path = Array.isArray(operation.path) ? operation.path : [];
  return [
    operationAsset(operation, "source_"),
    ...path.map((entry) => {
      if (!entry || typeof entry !== "object" || Array.isArray(entry)) return "Unknown asset";
      const asset = entry as ClassicOperation;
      return assetLabel(textField(asset, "asset_type"), textField(asset, "asset_code"), textField(asset, "asset_issuer"));
    }),
    operationAsset(operation, ""),
  ];
}
...
function TopBar({ network, kind, value, onBack }: { network: string; kind: "tx" | "account" | "contract" | "ledger"; value: string | number; onBack?: () => void }) {
  ...
  return (
    <div className="rx-topbar">
      {onBack ? <button className="rx-back-btn" type="button" onClick={onBack}><ArrowLeft /> Back to project</button> : <Link className="rx-back-btn" href={homeHref}><ArrowLeft /> Back to project</Link>}
...
function ExplorerDesignStyles() {
  ...
  `}</style><style>{`
    ...
    .rx-failed{display:flex;align-items:center;gap:6px;color:var(--red)}
    .rx-failed svg{width:13px;height:13px}
    .rx-swap-intent{padding:18px 16px}
    .rx-swap-accounts,.rx-swap-limits{display:flex;align-items:center;justify-content:space-between;gap:18px;flex-wrap:wrap;color:var(--text-dim);font-size:11.5px}
    .rx-swap-route{display:flex;align-items:center;gap:8px;margin:20px 0;padding:15px;border:1px solid var(--border);border-radius:6px;background:var(--panel);overflow-x:auto}
    .rx-swap-route>span{display:flex;align-items:center;gap:8px;white-space:nowrap}
    .rx-swap-route strong{font:600 11px ui-monospace,monospace;color:var(--text)}
    .rx-swap-route svg{width:13px;color:var(--text-faint)}
    .rx-swap-limits strong{margin-left:5px;color:var(--text);font-weight:650}
    .rx-failure-note{display:flex;align-items:flex-start;gap:8px;margin-top:16px;padding:10px 11px;border:1px solid color-mix(in srgb,var(--red) 42%,var(--border));border-radius:5px;background:color-mix(in srgb,var(--red) 8%,var(--bg));color:#f3a2a2;font-size:11.5px;line-height:1.5}
    .rx-failure-note svg{width:14px;flex-shrink:0;margin-top:1px}
  `}</style><style>{`.rx-root{--bg:#121212;--panel:#181818;--border:#2b2b2b;--text:#f5f5f5;--text-dim:#a1a1a1;--text-faint:#707070;--blue:#707070;--green:#a3ff5f;--orange:#a3ff5f;--red:#fb7185;--purple:#52525b;--purple-hover:#71717a}.rx-btn-purple{background:var(--purple);border-color:var(--purple)}.rx-tag.sender{background:rgba(251,113,133,.15)}.rx-tag.receiver{background:rgba(163,255,95,.15)}.rx-token-ic,.rx-sw-net-dot{background:linear-gradient(135deg,#707070,#a1a1a1)}.rx-flow-amount{background:rgba(163,255,95,.12)}`}</style></>;
}
    ...
    <div className={`rx-root${embedded ? " rx-root-embedded" : ""}`}>
      ...
      <div className="rx-subheader">
        <div className="rx-subheader-left">
          {embedded && (
            <button className="rx-embedded-back" type="button" aria-label="Back to explorer" onClick={goBack}>
              <ChevronLeft />
            </button>
          )}
          <div className="rx-subheader-title">
            <span className="rx-crumb-light rx-crumb-with-icon"><Receipt />Transaction</span>
            <span className="rx-subheader-hash mono" title={tx.hash}>
              {truncateEntity(tx.hash, 12, 10)}
              <button
                className="entity-copy-inline"
                data-copied={hashCopied ? "true" : "false"}
                type="button"
                aria-label="Copy transaction hash"
                title={hashCopied ? "Copied" : "Copy transaction hash"}
                onClick={() => void copyHash()}
              >
                {hashCopied ? <Check /> : <Copy />}
              </button>
            </span>
          </div>
        </div>
...
function SummaryPanel({ tx, network, devMode }: { tx: ExplorerTxDetail; network: string; devMode: boolean }) {
  const accountScoped = isAccountScopedOperation(tx.operation_type);
  const pathPayment = operationRecords(tx).find((operation) => ["path_payment_strict_receive", "path_payment_strict_send"].includes(textField(operation, "type") ?? ""));
  return <>
    {pathPayment && <div className="rx-card"><div className="rx-card-header-row"><span>{tx.status.toLowerCase() === "failed" ? "Attempted swap" : "Swap route"}<span className="rx-dim"> ({textField(pathPayment, "type")?.replaceAll("_", " ")})</span></span></div><div className="rx-swap-intent"><div className="rx-swap-accounts"><span>Initiated by <EntityAnchor network={network} value={textField(pathPayment, "from") ?? tx.source_account} type="address" /></span><span>Recipient <EntityAnchor network={network} value={textField(pathPayment, "to") ?? tx.source_account} type="address" /></span></div><div className="rx-swap-route">{attemptedPath(pathPayment).map((asset, index, route) => <span key={`${asset}-${index}`}><strong>{asset}</strong>{index < route.length - 1 && <ArrowRight />}</span>)}</div><div className="rx-swap-limits"><span>Source limit <strong>{textField(pathPayment, "source_max") ?? textField(pathPayment, "source_amount") ?? "Not specified"} {operationAsset(pathPayment, "source_")}</strong></span><span>Requested destination <strong>{textField(pathPayment, "amount") ?? textField(pathPayment, "destination_amount") ?? "Not specified"} {operationAsset(pathPayment, "")}</strong></span></div>{tx.status.toLowerCase() === "failed" && <div className="rx-failure-note"><X />The path was requested, but the transaction failed and Horizon reports no applied effects. No balances moved.</div>}</div></div>}
...
function ContractsPanel({ contracts, network }: { contracts: string[]; network: string }) {
  return <div className="rx-card"><div className="rx-card-header-row"><span>Contracts involved <span className="rx-dim">({contracts.length})</span></span></div><div className="rx-contract-grid">
    {contracts.length ? contracts.map((contract, index) => <div className="rx-contract-card" key={contract}><div className="rx-cc-head"><EntityIdenticon className="rx-cc-icon" value={contract} kind="contract" size={24} /><div><div className="rx-cc-name">Soroban contract</div><div className="rx-cc-type mono"><EntityAnchor network={network} value={contract} type="address" /></div></div></div><div className="rx-cc-meta"><div>Type<b>Contract</b></div><div>Trace refs<b>{index + 1}</b></div><div>Network<b>{network}</b></div></div></div>) : <div className="rx-empty">No contracts decoded.</div>}
  </div></div>;
}
...
export function WalletExplorerDesign({ account, network, address, label, environmentId, embedded = false, environmentScoped = false, onBack, onFund, onWatch, onSend }: { account: ExplorerAccountDetail; network: string; address: string; label?: string | null; environmentId?: string; embedded?: boolean; environmentScoped?: boolean; onBack?: () => void; onFund?: () => void; onWatch?: () => void; onSend?: () => void }) {
  ...
  const router = useRouter();
  useExplorerScrollTop(!environmentScoped);
  const handleBack = () => {
    if (onBack) {
      onBack();
      return;
